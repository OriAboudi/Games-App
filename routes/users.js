...
const { extname } = require("path"); const router = express.Router();
...
router.patch("/role/", authAdmin, async (req, res) => {
  try {
    ...
    if (user_id == req.tokenData._id || user_id == "63b13b337d99931225132187") {
...
//   try {
//     let name = req.tokenData._id;
//     let myFile = req.files.myFile;
//     if (myFile) {
//       let exts_arr = [".png", ".jpg", ".jpeg"];
//       if (!exts_arr.includes(path.extname(myFile.name))) {
//         return res.status(400).json({ msg: "file must be image" })
//       }
//       else if (myFile.size >= 1024 * 1024 * 5) {
//         return res.status(400).json({ msg: "file too big maximum 5MB" })
//       }
//       myFile.mv('public/images/' + name + path.extname(myFile.name), (error) => {
//         if (error) return res.status(400).json({ error })
//       })

//       let user = await UserModel.findOne({ _id: req.tokenData._id });
//       user.image =req.tokenData._id + path.extname(myFile.name);
//       await user.save();
...
router.delete("/:idDel", authAdmin, async (req, res) => {
  try {
    ...
    // 63b13b337d99931225132187 -> id of admin@walla.com (super admin)
    if (idDel == req.tokenData._id || idDel == "63b13b337d99931225132187") {
      return res.status(401).json({ msg: "You try to change yourself or the superadmin , anyway you are stupid!" })
    }
